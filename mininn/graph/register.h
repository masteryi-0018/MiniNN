...
class KernelFactory {
 public:
  void register_creator(Op type,
                        std::function<std::shared_ptr<Kernel>()> func) {
...
#define REGISTER_OP(op_type_, OpClass)                     \
  static OpRegistrar op_type_##_registrar(op_type_, []() { \
...
#define REGISTER_KERNEL(kernel_type_, KernelClass)                     \
  static KernelRegistrar kernel_type_##_registrar(kernel_type_, []() { \
