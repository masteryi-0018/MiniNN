...
void gemm_func(float* input_buffer, float* weight_buffer, float* bias_buffer,
               float* out_buffer, std::vector<int> input_shape,
  ...
  for (int i = 0; i < M; ++i) {
    for (int j = 0; j < N; ++j) {
      ...
      for (int k = 0; k < K; ++k) {
        ...
        float b =
            transpose_B ? weight_buffer[j * K + k] : weight_buffer[k * N + j];
        sum += a * b;
      }
  ...
  for (int i = 0; i < M; ++i) {
    for (int j = 0; j < N; ++j) {
      ...
      if (bias_buffer != nullptr && bias_shape.size() > 0) {
        if (bias_shape[0] == N) {
          value += bias_buffer[j];
        } else if (bias_shape.size() == 2 && bias_shape[0] == M &&
                   bias_shape[1] == N) {
...
void GemmCompute::run() {
  ...
  gemm_func(input_buffer, weight_buffer, bias_buffer, out_buffer, input_shape,
            weight_shape, bias_shape, alpha, beta, transB);
