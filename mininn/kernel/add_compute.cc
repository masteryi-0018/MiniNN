...
#include "mininn/backend/metal/metal.h"
...
void add_func(float* x_buffer, float* y_buffer, float* out_buffer, int start,
              int end) {
...
void add_func_multi_threads(float* x_buffer, float* y_buffer, float* out_buffer,
                            int size) {
...
void AddCompute::run() {
  ...
  end_time = std::chrono::high_resolution_clock::now();
  elapsed_seconds = end_time - start_time;
  LOG(INFO) << "Elapsed time in neon: " << elapsed_seconds.count()
            << " seconds";
#endif  // WITH_NEON

#ifdef WITH_METAL
  start_time = std::chrono::high_resolution_clock::now();
  metal_add_wrapper(x_buffer, y_buffer, out_buffer, size);
  end_time = std::chrono::high_resolution_clock::now();
  elapsed_seconds = end_time - start_time;
  LOG(INFO) << "Elapsed time in metal: " << elapsed_seconds.count()
            << " seconds";
#endif  // WITH_METAL
