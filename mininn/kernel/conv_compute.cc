...
void conv_func(float* input_buffer, float* weight_buffer, float* bias_buffer,
               float* out_buffer, std::vector<int> dilations,
               std::vector<int> group, std::vector<int> kernel_shape,
  ...
  // for (int i = 0; i < out_channels * in_group_size * kernel_height *
  // kernel_width; ++i) {
  //     // use std::setprecision to control the number of decimal places
  //     std::cout << "Float: " << std::setprecision(32) << std::fixed <<
  //     weight_buffer[i] << std::endl; break;
  // }
  ...
  float* padded_input =
      new float[batch_size * in_channels * padded_height * padded_width];
  std::memset(
      padded_input, 0,
      batch_size * in_channels * padded_height * padded_width * sizeof(float));
    ...
    for (int c = 0; c < in_channels; ++c) {
      for (int h = 0; h < input_height; ++h) {
        for (int w = 0; w < input_width; ++w) {
          ...
          int src_idx =
              ((n * in_channels + c) * input_height + h) * input_width + w;
          int dst_idx = ((n * in_channels + c) * padded_height + padded_h_pos) *
                            padded_width +
                        padded_w_pos;
      ...
      for (int oc = 0; oc < out_group_size; ++oc) {
        ...
        for (int oh = 0; oh < output_height; ++oh) {
          for (int ow = 0; ow < output_width; ++ow) {
            ...
            for (int ic = 0; ic < in_group_size; ++ic) {
              ...
              for (int kh = 0; kh < kernel_height; ++kh) {
                for (int kw = 0; kw < kernel_width; ++kw) {
                  ...
                  int input_idx =
                      ((n * in_channels + in_c) * padded_height + in_h_pos) *
                          padded_width +
                      in_w_pos;
                  int weight_idx =
                      ((out_c * in_group_size + ic) * kernel_height + kh) *
                          kernel_width +
                      kw;
            ...
            int out_idx = ((n * out_channels + out_c) * output_height + oh) *
                              output_width +
                          ow;
...
void ConvCompute::run() {
  ...
  conv_func(input_buffer, weight_buffer, bias_buffer, out_buffer, dilations,
            group, kernel_shape, pads, strides, input_shape, out_shape);
