...
void initOpenGL() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "OpenGL Vector Add", nullptr, nullptr);
    if (!window) {
        glfwTerminate();
...
GLuint createBufferObject(const std::vector<float>& data, GLuint bindingPoint) {
    ...
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, buffer); // Bind the buffer to the GL_SHADER_STORAGE_BUFFER target
    glBufferData(GL_SHADER_STORAGE_BUFFER, data.size() * sizeof(float), data.data(), GL_STATIC_DRAW); // Upload data to the buffer
...
int main() {
    ...
    glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);  // Ensure completion of writes

    // Retrieve results from buffer C
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, bufferC);
    float* ptr = (float*)glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, C.size() * sizeof(float), GL_MAP_READ_BIT);
