        ...
        for output in node.output:
            if not any(o.name == output for o in model.graph.output):
                if get_dtype(output, onnx_model_shape) == TensorProto.FLOAT:
                    onnx_model_shape.graph.output.append(helper.make_tensor_value_info(output, TensorProto.FLOAT, None))
                elif get_dtype(output, onnx_model_shape) == TensorProto.INT64:
                    onnx_model_shape.graph.output.append(helper.make_tensor_value_info(output, TensorProto.INT64, None))
