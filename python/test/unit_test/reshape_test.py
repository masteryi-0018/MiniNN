...
def make_model(input_shape=[1, 1280, 1, 1], output_shape=[1, 1280], shape=[1, 1280]):
    ...
    input1 = helper.make_tensor(
        name="input1", data_type=TensorProto.INT64, dims=shape, vals=shape_value
    )

    output0 = helper.make_tensor_value_info("output0", TensorProto.FLOAT, output_shape)

    reshape_node = helper.make_node(
        "Reshape", inputs=["input0", "input1"], outputs=["output0"], name="reshape_node"
    )

    graph = helper.make_graph(
        [reshape_node], "reshape_model", [input0], [output0], initializer=[input1]
    )
...
def test_reshape():
    ...
    model_path = make_model(
        input_shape=input_shape, output_shape=output_shape, shape=shape
    )
